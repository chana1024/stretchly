    ...
    (event, commandLine, workingDirectory, commandLineArguments) => {
      ...
      switch (cmd.command) {
        ...
        case "toggle":
          ...
          break;
        case "toggleBreak":
          log.info("Stretchly: toggle Breaks html");
          startBreak();
          break;
        case "pause": {
          ...
          if (ms === -1) {
            log.error(
              "Stretchly: error when parsing duration to ms because of unvalid value"
            );
            return;
          }
          pauseBreaks(ms);
...
function initialize(isAppStart = true) {
  ...
  if (settings.get("resumeBreaksShortcut") !== "") {
    ...
    if (!resumeBreaksShortcut) {
      ...
      log.info(
        `Stretchly: resumeBreaksShortcut registration succesful (${settings.get(
  ...
  if (settings.get("pauseBreaksShortcut") !== "") {
    ...
    if (!pauseBreaksShortcut) {
      ...
      log.info(
        `Stretchly: pauseBreaksShortcut registration succesful (${settings.get(
          "pauseBreaksShortcut"
        )})`
      );
    }
...
function startMicrobreak() {
  ...
  const idea =
    nextIdea || (settings.get("ideas") ? microbreakIdeas.randomElement : [""]);
  nextIdea = null;
