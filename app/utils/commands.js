...
const allOptions = {
  ...
  duration: {
    ...
    short: "-d",
    description:
      "Specify duration for pausing breaks (Pause only) [indefinitely|until-morning|HHhMMm|HHh|MMm|MM]",
    withValue: true,
  },
};
...
class Command {
  ...
  parse(input) {
    ...
    if (!this.supported[this.command]) {
      log.error(
        `Stretchly${this.isFirstInstance ? "" : " 2"}: command '${
          this.command
        }' is not supported`
      );
      return;
  ...
  cmdHelp() {
    ...
    const cmds = Object.keys(this.supported).map(
      (key) =>
        `${key}${
          this.supported[key].options === undefined ? "" : ` ${options}`
        }`
    );
    const longuest = cmds.reduce(
    ...
    part = Object.keys(this.supported).reduce((acc, key) => {
      ...
      const line = `stretchly ${cmds[i]}${" ".repeat(padding)} ${
        this.supported[key].description
      }`;
      i++;
      return `${acc}\n\t${line}`;
    }, part);
  ...
  optionsHelp() {
    ...
    const longuest = Object.keys(allOptions).reduce(
      (acc, key) =>
        acc > allOptions[key].long.length ? acc : allOptions[key].long.length,
      0
    );
    ...
    part = Object.keys(allOptions).reduce((acc, key) => {
      ...
      const line = `${opt.short}, ${opt.long}${" ".repeat(padding)} ${
        opt.description
      }`;
      return `${acc}\n\t${line}`;
    }, part);
  ...
  examplesHelp() {
    ...
    const longuest = allExamples.reduce(
      (acc, cur) => (acc > cur.cmd.length ? acc : cur.cmd.length),
      0
    );
  ...
  help() {
    console.log(
      [this.cmdHelp(), this.optionsHelp(), this.examplesHelp()].join("")
    );
  }
}
